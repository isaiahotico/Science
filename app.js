...
window.login = async () => {
    ...
    if (!user.startsWith('@') || user.length < 4) return alert("Enter valid Telegram @username");
...
function initApp() {
    ...
    // Global Monetag Interstitial (Every 3 minutes as requested)
    triggerGlobalAd();
    setInterval(triggerGlobalAd, 180000); 
...
window.renderTasks = () => {
    ...
    taskLinks.forEach((link, index) => {
        ...
        if (isCooldown) {
            ...
            sub = `<span class="text-red-500 font-bold text-[10px] uppercase">${h}h ${m}m ${s}s left</span>`;
        }

        card.innerHTML = `
            <div>
                <p class="font-bold text-[10px] uppercase text-gray-400">Mine Link #${index+1}</p>
                ${sub}
            </div>
            ${!isCooldown ? `<button onclick="openTask(${index},'${link}')" class="bg-blue-600 text-white px-6 py-2 rounded-2xl font-black text-[10px] tracking-widest uppercase shadow-lg shadow-blue-100">Mine</button>` : ''}
        `;
...
function startTaskTimer() {
    ...
    timerInterval = setInterval(() => {
        if (!isPaused) {
            ...
            // Auto pause every 5 seconds (Requires Prompt Click)
...
function updateTimerUI() {
    ...
    const adTag = document.getElementById('ad-status');
    const nextAdIn = timerSeconds % 10;
    adTag.innerText = `Dual Ad in ${nextAdIn === 0 ? 10 : nextAdIn}s`;
}
...
window.submitReferral = async () => {
    ...
    if (code === currentUser.referralCode || currentUser.referredBy) return alert("Operation Invalid");
...
function listenChat() {
    onValue(query(ref(db, 'chats'), limitToLast(15)), (snap) => {
        ...
        snap.forEach(c => {
            const m = c.val();
            box.innerHTML += `<div class="bg-gray-100 p-3 rounded-3xl w-fit max-w-[85%] shadow-sm">
                <p class="text-[8px] font-black text-blue-600 uppercase mb-1">${m.user}</p>
                <p class="text-xs font-medium">${m.text}</p>
            </div>`;
        });
...
function listenLeaderboard() {
    onValue(query(ref(db, 'users'), orderByChild('balance'), limitToLast(10)), (snap) => {
        ...
        items.reverse().forEach((u, i) => {
            list.innerHTML += `<div class="flex justify-between p-4 text-xs font-black">
                <span class="text-gray-500">${i+1}. ${u.username}</span>
                <span class="text-blue-600 italic">₱${u.balance.toFixed(3)}</span>
            </div>`;
        });
    });
}
...
window.requestWithdraw = async () => {
    ...
    alert("Request queued for approval!");
};
        ...
        onValue(ref(db, 'withdrawals'), (snap) => {
            ...
            snap.forEach(c => {
                if (c.val().status === 'pending') {
                    const w = c.val();
                    adminBox.innerHTML += `<div class="bg-white p-4 rounded-3xl border-l-4 border-blue-600 shadow-sm">
                        <p class="font-black text-xs">${w.username} - ₱${w.amount}</p>
                        <p class="text-[10px] text-gray-400">${w.method}: ${w.details}</p>
                        <button onclick="approve('${c.key}')" class="bg-blue-600 text-white px-8 py-2 rounded-xl text-[10px] font-black mt-2">APPROVE</button>
                    </div>`;
                }
            });
        });
