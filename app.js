...
// --- DATABASE CONFIGURATION (PAPERHOUSE NEW CREDENTIALS) ---
...
window.onload = () => {
    ...
    // Auto Show Interstitial with configured intervals (4 minutes delay loop)
    runInterstitialLoop();
...
// Trigger ad immediately on open, then repeat every 4 minutes (240000ms)
...
function buySampaguita() {
    ...
    // Check garden holding capacity limit
    if (activePlants >= MAX_PLANTS_LIMIT) {
        alert(`You have reached your garden limit of ${MAX_PLANTS_LIMIT} plants! Claim or wait for existing plants to expire.`);
        return;
    }

    const now = Date.now();
    // Reset hourly limit counter if 1 hour has elapsed
    ...
    if (userData.adsCount >= 10) {
        alert("Hourly limits reached! You can watch a maximum of 10 ads per hour.");
        return;
    }
    ...
    if (typeof show_10555663 === 'function') {
        show_10555663().then(() => {
            ...
            // Fallback system if ad platform is blocked/fails to load for developer testing
            alert("Ad playback complete!");
            executePlantPurchase();
        });
    } else {
        alert("Loading Ad Networks...");
        executePlantPurchase(); // Emergency local fallback validation
    }
}
...
function executePlantPurchase() {
    ...
    const updates = {};
    updates[`users/${UID}/plants/${plantId}`] = newPlantData;
    updates[`users/${UID}/adsCount`] = (userData.adsCount || 0) + 1;
    updates[`users/${UID}/lastAdHour`] = now;

    db.ref().update(updates).then(() => {
        alert("🎉 Sampaguita flower successfully planted in your garden!");
    });
}
...
function renderPlants() {
    ...
    if (plantsList.length === 0) {
        ...
        <div class="text-center py-16 bg-slate-900/40 rounded-3xl border border-slate-800/50 border-dashed">
            ...
            <p class="text-slate-600 text-[11px] mt-1">Buy plants above to begin automatically generating real earnings.</p>
        </div>`;
    ...
    sortedPlants.forEach(p => {
        ...
        // Exact real-time earnings generated since last claim
...
function claimSpecificPlant(pid) {
    ...
    if (earnings <= 0) {
        alert("Insufficient accrued yields to execute claim.");
        return;
    }

    // Trigger Rewarded Interstitial before processing claim credit
    if (typeof show_10555746 === 'function') {
        show_10555746().then(() => {
            processClaimCredit(pid, earnings, finalClaimTime, isExpired);
        }).catch(() => {
            processClaimCredit(pid, earnings, finalClaimTime, isExpired); // Testing bypass protection
        });
    } else {
        processClaimCredit(pid, earnings, finalClaimTime, isExpired);
    }
...
function processClaimCredit(pid, earnings, finalClaimTime, isExpired) {
    ...
    userRef.child('balance').transaction(currentBal => {
        ...
        if (committed) {
            if (isExpired) {
                // Remove expired plant contract from the database
                db.ref(`users/${UID}/plants/${pid}`).remove();
            } else {
                // Update claim timestamp dynamically
            ...
            // Pay 5% Commission if Referrer is set
            ...
            alert(`🎉 Success! ₱${earnings.toFixed(5)} has been securely added to your balance.`);
        } else {
            alert("Double transaction detected! Please retry.");
        }
    });
}
...
function switchTab(tab) {
    ...
    // Reset active visual classes on bottoms
    ...
    if (tab === 'withdraw') {
        ...
        container.innerHTML = `
            <div class="glass-panel p-6 rounded-[2rem] text-center space-y-4">
                <p class="text-xs text-slate-400 leading-relaxed">Invite partners using your link code and earn <span class="text-emerald-400 font-bold">5% commission</span> instantly every time they claim mining outputs!</p>
...
function requestWithdrawal() {
    ...
    if (recipient === "") {
        alert(`You must specify a valid destination ${method} details.`);
        return;
    }
