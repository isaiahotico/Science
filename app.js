...
const CONTRACT_MS = 7 * 24 * 60 * 60 * 1000; // 7 days contract lifetime
const PROFIT = 0.05;
...
function renderUI() {
    ...
    pagePlants.forEach((p, i) => {
        ...
        tr.className = "hover:bg-slate-50 transition font-mono text-[11px]";
        tr.innerHTML = `
...
// --- SECURE ADMINISTRATIVE SUITE (Propetas12) ---
window.adminCheck = () => { 
    if (prompt("Enter Administrative Key Password:") === "Propetas12") { 
        tab('admin'); 
        loadAdmin(); 
    } else {
...
async function loadAdmin() {
    ...
    snap.forEach(s => {
        ...
        // Format document transaction date safely
        const reqDate = d.time ? new Date(d.time.seconds * 1000).toLocaleString() : "Processing Timestamp...";

        el.innerHTML = `
            <div class="space-y-1.5">
                <div class="flex justify-between items-center text-[9px] font-mono font-bold text-slate-400">
                    <span>TX ID: ${s.id}</span>
                </div>
                ...
                <p><b>Amount:</b> <span class="font-black text-emerald-600 font-mono text-sm">₱${d.amt.toFixed(2)}</span></p>
                <p><b>Account Gateway:</b> <span class="font-extrabold uppercase text-indigo-600 bg-indigo-50 px-1.5 py-0.5 rounded">${d.method}</span></p>
                <p><b>Recipient Account:</b> <span class="font-mono bg-emerald-50 px-1.5 py-0.5 rounded text-emerald-800 font-black tracking-wide select-all">${d.dest}</span></p>
                <p><b>Requested Date/Time:</b> <span class="text-slate-500 font-semibold">${reqDate}</span></p>
            </div>
...
window.pay = async (pid, status, target, amt) => { 
    ...
    if (status === 'denied' && target && amt) {
        // Safe rollback refund logic for declined withdrawals
