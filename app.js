...
async function init() {
    ...
    if(!snap.exists()) {
        ...
        // Update real-time name if it changed in Telegram
    ...
    onSnapshot(uRef, (d) => {
        ...
        if(data.referredBy) {
            document.getElementById('refInputSection').innerHTML = `<p style="color:green">Invited by @${data.referredBy}</p>`;
        }
