...
function renderUI() {
    ...
    const totalPlants = user.plants ? user.plants.length : 0;
    const maxPage = Math.ceil(totalPlants / ITEMS_PER_PAGE) || 1;
    if (curPage > maxPage) curPage = maxPage; // Adjust if plants were deleted
    ...
    if (pagePlants.length === 0) {
        tbody.innerHTML = `<tr><td colspan="5" class="text-center py-10 font-bold text-slate-400">GARDEN IS EMPTY</td></tr>`;
        return;
    }
    ...
    pagePlants.forEach((p, i) => {
        ...
        tr.innerHTML = `
            ...
            <td class="px-4 py-3 text-right">
                <button onclick="claim(${globalIdx})" class="bg-slate-800 text-white px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-tighter">Claim (663)</button>
            </td>
        `;
...
window.buyFlower = async () => {
    ...
    if (hourly.length >= 10) return alert("Limit reached: Only 10 plants per hour allowed.");
...
window.adminCheck = () => { if (prompt("Admin Key:") === "Propetas12") { tab('admin'); loadAdmin(); } };
...
async function loadAdmin() {
    ...
    snap.forEach(s => {
        ...
        el.className = "glass p-4 rounded-xl text-xs space-y-2";
        el.innerHTML = `<p><b>User:</b> ${d.uid}<br><b>Amount:</b> ₱${d.amt}<br><b>Dest:</b> ${d.dest}</p>
            <div class="flex gap-2">
                <button onclick="pay('${s.id}', 'paid')" class="bg-emerald-500 text-white p-2 rounded-lg flex-1">Approve</button>
                <button onclick="pay('${s.id}', 'denied', '${d.uid}', ${d.amt})" class="bg-red-500 text-white p-2 rounded-lg flex-1">Deny</button>
            </div>`;
        list.appendChild(el);
    });
}
...
window.pay = async (pid, status, target, amt) => { 
    ...
    if (status === 'denied') await updateDoc(doc(db, "users", target), { balance: increment(amt) });
    loadAdmin(); 
};
