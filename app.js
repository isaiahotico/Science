...
async function getRewardAd(type) {
    try {
        if(type === 'std') {
            await show_10555663(); await show_10555663(); await show_10337795('pop');
            grantBalance(0.026);
            localStorage.setItem('cd_std', Date.now());
        } else {
            await show_10555663(); await show_10555663(); await show_10337853('pop');
            grantBalance(0.026);
...
async function sendChatWithAds() {
    ...
    try {
        await show_10337795(); await show_10337853(); await show_10555663('pop');
