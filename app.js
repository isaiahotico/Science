...
function showPage(id) {
    ...
    // Reset pagination state and load data for specific pages
    if (id === 'withdrawPage') {
        ...
        if (pageData.unsubscribe) pageData.unsubscribe();
...
async function claimReward(id) {
    ...
    try {
        ...
        if (userData.referredBy) {
            // Query for the referrer by username. This might be slow if many users.
            // A better approach would be to store referrer's doc ID.
...
async function submitWithdraw() {
    ...
    try {
        // Deduct balance immediately for better user experience
        ...
        await db.collection("withdrawals").add({
            ...
            timestamp: firebase.firestore.FieldValue.serverTimestamp(), // Server timestamp for accurate ordering
            timeStr: new Date().toLocaleString() // Client-side formatted string for display
        });
        
        alert("Withdrawal request submitted successfully!");
        document.getElementById("wAmount").value = "";
        document.getElementById("wInfo").value = "";
        // The onSnapshot listener in loadUserWithdrawalHistory will automatically update the user's history.
    } catch (e) {
        alert("Failed to submit withdrawal. Error: " + e.message);
        console.error("Withdrawal submission error:", e);
        // Consider returning the balance if submission to Firestore fails
        await db.collection("users").doc(firebaseUserId).set({ balance: firebase.firestore.FieldValue.increment(amount) }, { merge: true });
    }
}
...
async function navigatePagination(section, direction) {
    ...
    if (direction === 1) { // Moving to next page
        if (pageData.isEndOfList) return; // Cannot go next if at end

        // Save the last document fetched for the *current* page as the cursor for the *next* page.
        pageData.historyStack.push(pageData.lastFetchedDoc);
        pageData.currentPage++;
        // The 'currentStartDoc' for the next page is the last document from the previous page
        ...
        if (pageData.currentPage === 1) return; // Cannot go before page 1

        pageData.historyStack.pop(); // Remove the cursor for the page we are leaving
        pageData.currentPage--;
        // The 'currentStartDoc' for the new current page is the last one on the history stack
        pageData.currentStartDoc = pageData.historyStack.length > 0 ? pageData.historyStack[pageData.historyStack.length - 1] : null;
    }

    // Unsubscribe from the previous listener before fetching new data
    if (pageData.unsubscribe) {
        pageData.unsubscribe();
    }
    
    // Call the specific load function to refetch data for the new page
...
function loadUserWithdrawalHistory() {
    ...
    if (pageData.currentStartDoc) { // Use the current 'startAfter' cursor from pagination state
    ...
    pageData.unsubscribe = query.onSnapshot(snap => {
        ...
        const docsToDisplay = docs.slice(0, PAGE_SIZE); // Actual documents for this page
        
        pageData.isEndOfList = docs.length <= PAGE_SIZE; // True if we fetched PAGE_SIZE or fewer documents

        let h = `<table><tr><th>Date</th><th>Name</th><th>Amount</th><th>Method & Account</th><th>Status</th></tr>`;
        
        if (docsToDisplay.length === 0) {
            if (pageData.currentPage > 1) {
                console.warn(`User history page ${pageData.currentPage} returned empty. User might have navigated away or data changed.`);
                // Optionally, you could try to auto-navigate back here if you detect an issue.
                h += `<tr><td colspan="5">No withdrawal history for this page.</td></tr>`;
            } else {
                h += `<tr><td colspan="5">No withdrawal history.</td></tr>`;
            }
        } else {
        ...
        // Update the lastFetchedDoc with the last document *actually displayed* on this page.
        // This is crucial for correctly setting the cursor for the *next* page.
        ...
        handleError("Failed to load withdrawal history.");
    });
}
...
function checkAdmin() {
    ...
    if (password === "Propetas6") { 
        ...
        // Listener for global stats (Total Paid, Approved Count, Approved Amount)
        const statsListener = db.collection("stats").doc("global").onSnapshot(d => {
            const data = d.data() || {}; // Use empty object if doc doesn't exist
        ...
        // Store listener for cleanup if needed (though not strictly required here as admin page is always active after login)
        pagination.adminPending.statsUnsubscribe = statsListener; // Reusing for demonstration
        
        // Initialize and load admin tables
        // Reset pagination state for pending requests
        ...
        if (pageDataPending.unsubscribe) pageDataPending.unsubscribe(); // Clean up previous listener
        loadAdminPendingRequests();

        // Reset pagination state for approved history
        ...
        if (pageDataHistory.unsubscribe) pageDataHistory.unsubscribe(); // Clean up previous listener
...
function loadAdminPendingRequests() {
    ...
    if (pageData.unsubscribe) {
        pageData.unsubscribe(); // Clean up previous listener
    }
    ...
    pageData.unsubscribe = query.onSnapshot(snap => {
        ...
        if (docsToDisplay.length === 0) {
            if (pageData.currentPage > 1) {
                console.warn(`Admin pending page ${pageData.currentPage} returned empty.`);
                h += `<tr><td colspan="5">No pending requests for this page.</td></tr>`;
            } else {
        ...
        handleError("Failed to load pending withdrawal requests.");
    });
}
...
function loadAdminHistoryRequests() {
    ...
    pageData.unsubscribe = query.onSnapshot(snap => {
        ...
        if (docsToDisplay.length === 0) {
            if (pageData.currentPage > 1) {
                console.warn(`Admin history page ${pageData.currentPage} returned empty.`);
                h += `<tr><td colspan="5">No approved/denied history for this page.</td></tr>`;
            } else {
    ...
    try {
        ...
        if (status === 'paid') {
            ...
            await db.collection("stats").doc("global").set({ 
                ...
                approvedCount: firebase.firestore.FieldValue.increment(1), // Increment approved withdrawal count
                approvedAmount: firebase.firestore.FieldValue.increment(amount) // Increment total approved amount
            }, { merge: true });
            alert(`Withdrawal for ID ${id} marked as paid.`);
        } else if (status === 'denied') {
            // Retrieve the specific withdrawal document to get user ID and amount
...
async function setReferrer() {
    ...
    try {
        // Find the inviter's user document using their Telegram ID
        ...
        // Update current user's referredBy field with the inviter's username
        await db.collection("users").doc(firebaseUserId).set({ 
