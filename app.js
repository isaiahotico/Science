...
const CONTRACT_MS = 7 * 24 * 60 * 60 * 1000; // 7-day contract lifetime
const PROFIT = 0.05;
    ...
    snap.forEach(s => {
        ...
        el.innerHTML = `
            <div class="space-y-1.5">
                ...
                <p><b>User ID:</b> <span class="font-mono bg-slate-100 px-1.5 py-0.5 rounded text-slate-700 font-bold">${d.uid}</span></p>
                ...
                <p><b>Destination Number:</b> <span class="font-mono bg-emerald-50 px-1.5 py-0.5 rounded text-emerald-800 font-black tracking-wide">${d.dest}</span></p>
            </div>
            <div class="flex gap-2">
                <button onclick="pay('${s.id}', 'paid')" class="bg-emerald-500 hover:bg-emerald-600 text-white py-2 rounded-xl flex-1 font-bold transition text-xs shadow-sm">Approve</button>
                <button onclick="pay('${s.id}', 'denied', '${d.uid}', ${d.amt})" class="bg-red-500 hover:bg-red-600 text-white py-2 rounded-xl flex-1 font-bold transition text-xs shadow-sm">Deny & Refund</button>
            </div>`;
        list.appendChild(el);
    });
}
...
window.pay = async (pid, status, target, amt) => { 
    ...
    if (status === 'denied' && target && amt) {
        // Safe Atomic rollback refund logic for denied cashouts
