...
const userId = tg?.initDataUnsafe?.user ? String(tg.initDataUnsafe.user.id) : "dev_test_user";
const username = tg?.initDataUnsafe?.user ? (tg.initDataUnsafe.user.username || tg.initDataUnsafe.user.first_name || "User") : "Guest";
...
// SYNC ACCOUNT DATA & FIX NaN
db.collection("users").doc(userId).onSnapshot(doc => {
    if (doc.exists) {
        const d = doc.data();
        userData = {
            username: d.username || username, // Ensure username is always set
            balance: Number(d.balance) || 0, // Convert to number, default to 0
        ...
        // Create new user with safe defaults if document doesn't exist
...
function showRandomInAppInterstitial() {
    ...
    if (typeof adFn === 'function') {
        // Use the specific inAppSettings provided by the user
        adFn({
            type: 'inApp',
            inAppSettings: {
                frequency: 2,
            ...
            // Ad might fail to show (e.g., no inventory, blocked by browser)
...
function runTask(id, zone, type = 'interstitial') {
    ...
    if (typeof adFn === 'function') {
        ...
        promise.then(() => {
            currentTaskId = id; // Store ID to prevent claiming wrong ad
...
async function claimReward(id) {
    ...
    try {
        ...
        localStorage.setItem(`cd_${id}_${userId}`, Date.now() + (cooldownSeconds * 1000));
        ...
        alert("Failed to claim reward. Please try again. Error: " + e.message);
...
function checkAdmin() {
    if (prompt("Owner Password:") === "Propetas6") { // Basic password check
        showPage('adminPage');
            ...
            snap.forEach(doc => {
                ...
                h += `<tr>
                        ...
                        <td>
                            ...
                            <button onclick="admProcess('${doc.id}','denied',0)" style="background:${getComputedStyle(document.documentElement).getPropertyValue('--danger')}; color:white; border:none; padding: 5px 10px; border-radius: 5px; cursor:pointer; margin-left:5px;">Deny</button>
                        </td>
                      </tr>`;
            });
...
async function admProcess(id, status, amountIfPaid) {
    try {
        await db.collection("withdrawals").doc(id).update({ status: status });
        if (status === 'paid') {
            await db.collection("stats").doc("global").set({ paid: firebase.firestore.FieldValue.increment(amountIfPaid) }, { merge: true });
...
async function setReferrer() {
    ...
    try {
        ...
        await db.collection("users").doc(userId).update({ referredBy: inviterUsername });
...
function checkCooldowns() {
    ...
    ids.forEach(id => {
        const endTime = localStorage.getItem(`cd_${id}_${userId}`);
