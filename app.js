...
function triggerGlobalAds() {
    ...
    if (!last || (Date.now() - last) > 180000) { // 3 minutes cooldown
...
window.startMainTask = () => {
    ...
    if (isMonetag) {
        ...
        else runTask(); // Fallback if ad function is not available
    } else {
        const bId = adsgramIds[Math.floor(Math.random() * adsgramIds.length)];
        // Ensure Adsgram is initialized, then show
        if (window.Adsgram && typeof window.Adsgram.init === 'function') {
            window.Adsgram.init({ blockId: bId }).show().then(runTask).catch(runTask);
        } else {
            runTask(); // Fallback if Adsgram SDK is not loaded
        }
    }
};
...
window.bonusAd = (bId, idx) => {
    if (window.Adsgram && typeof window.Adsgram.init === 'function') {
        window.Adsgram.init({ blockId: bId }).show().then(async () => {
            ...
            distributeReward(reward);
        }).catch(() => alert("Ad not ready or skipped. Try again."));
    } else {
        alert("Adsgram SDK not loaded. Cannot show bonus ad.");
    }
};
...
// Admin queue now explicitly shows username, amount, method, and address
        ...
        for (let id in data) {
            if (data[id].status === "pending") {
                ...
                <div class="bg-black/60 p-4 rounded-2xl border border-white/10 text-[10px]">
                    <div class="mb-1"><b>User:</b> ${data[id].user}</div>
                    <div class="mb-1"><b>Method:</b> ${data[id].method}</div>
                    <div class="mb-2"><b>Target:</b> <span class="text-blue-400 font-bold">${data[id].address}</span></div>
                    <div class="flex justify-between items-center">
                        <span class="text-green-500 font-black">₱${data[id].amount}</span>
...
window.postAdminLink = () => {
    ...
    } else alert("Please enter a valid URL (must include http/https).");
};
...
window.applyReferral = async () => {
    ...
    if (c === userData.referralCode) return alert("Cannot refer yourself.");
    ...
    for (let id in all) {
        if (all[id].referralCode === c) {
            ...
            document.getElementById("refInput").value = ""; // Clear input
            break;
        }
    }
    if (!found) alert("Referral code not found.");
};
...
// Initial app load
init();
// Set user offline when they close the app
