...
let uBalance = 0;
let currentRoomId = null; // Tracks the currently active chat room
let unsubscribeChatListener = null; // To manage chat listeners
...
async function init() {
    ...
    if(!snap.exists()) {
        ...
        // Ensure username is always up-to-date with Telegram
        await updateDoc(uRef, { username: liveName });
    }
...
window.nav = (id) => {
    ...
    // Special handling for chat page to ensure room list is shown first
    if (id === 'chatPage') {
        document.getElementById('chatRoomsList').style.display = 'block';
        document.getElementById('roomChatContent').style.display = 'none';
        // Ensure previous chat listener is stopped when navigating out of a specific room
...
// Task System Setup (remains unchanged)
const tasks = {
...
function enterRoom(roomId, roomName) {
    ...
    // Hide room list, show specific chat content
...
window.backToRooms = () => {
    ...
    if (unsubscribeChatListener) {
        unsubscribeChatListener(); // Stop listening to the previous room's messages
        unsubscribeChatListener = null;
    }
};
...
function loadRoomMessages(roomId) {
    ...
    // If there's an active listener, unsubscribe it first
    if (unsubscribeChatListener) {
        unsubscribeChatListener();
    }

    const messagesContainer = document.getElementById('chatMessages');
    messagesContainer.innerHTML = ''; // Clear messages immediately when loading a new room
    ...
    unsubscribeChatListener = onSnapshot(roomMessagesQuery, (snap) => {
        messagesContainer.innerHTML = ''; // Clear before repopulating
        snap.forEach(doc => {
            const m = doc.data();
