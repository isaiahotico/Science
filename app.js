...
async function submitWithdraw() {
    ...
    try {
        ...
        // onSnapshot listener in loadUserWithdrawalHistory will automatically update
    } catch (e) {
...
async function navigatePagination(section, direction) {
    ...
    if (direction === 1) { // Moving to next page
        ...
        pageData.historyStack.push(pageData.lastFetchedDoc); // Save last doc of current page for next page's startAfter
        pageData.currentPage++;
        pageData.currentStartDoc = pageData.historyStack[pageData.historyStack.length - 1]; // New startAfter is the one we just pushed

    } else if (direction === -1) { // Moving to previous page
        if (pageData.currentPage === 1) return; // Cannot go before page 1

        pageData.historyStack.pop(); // Remove the startAfter cursor for the page we are leaving
        pageData.currentPage--;
        pageData.currentStartDoc = pageData.historyStack.length > 0 ? pageData.historyStack[pageData.historyStack.length - 1] : null; // New startAfter (could be null if back to page 1)
    }
    ...
    // Call the specific load function
...
function loadUserWithdrawalHistory() {
    ...
    if (pageData.currentStartDoc) { // Use the current 'startAfter' cursor
    ...
    pageData.unsubscribe = query.onSnapshot(snap => {
        ...
        if (docsToDisplay.length === 0 && pageData.currentPage > 1) {
            console.warn(`User history page ${pageData.currentPage} returned empty, navigating back.`);
            // This situation usually happens if all docs on the current page were deleted.
            // Rather than auto-navigating, we'll just show it empty and let the user navigate.
            // If the issue is an invalid currentStartDoc, the navigation logic itself needs to be robust.
        ...
        // Store the last fetched document of this *current page*
        // This will be used as the 'startAfter' cursor for the *next* page if user navigates forward.
