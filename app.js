...
async function init() {
    ...
    document.getElementById('refLink').value = `https://t.me/Key_52_bot/app?startapp=${username}`;
...
function renderTasks() {
    Object.keys(TASKS).forEach(key => {
        ...
        for (let i = 0; i < conf.count; i++) {
            ...
            div.className = 'card';
            div.innerHTML = `
                <button class="btn-task" id="btn-${id}" onclick="triggerAd('${key}', '${id}', ${i})">🤑🍍${conf.label} #${i+1}🍍🤑</button>
...
window.navTo = (id) => {
    ...
    if(TASKS[key] && TASKS[key].auto) {
        const autoAd = window[`show_${TASKS[key].auto}`];
        if(autoAd) autoAd({
...
window.submitWithdraw = async (method) => {
    ...
    if (!info) return alert("Please enter payment details");
    ...
    alert("Request Sent! Manual approval required by owner.");
};
...
function fetchHistory() {
    ...
    onSnapshot(q, (snap) => {
        ...
        const sliced = full.slice((userPage-1)*PAGE_SIZE, userPage*PAGE_SIZE);
...
/* ================= ADMIN ================= */
window.openAdmin = () => {
    if (prompt("Admin Password:") === "Propetas6") { navTo('page-admin'); fetchAdmin(); }
};

function fetchAdmin() {
    const q = query(collection(db, "withdrawals"), orderBy("timestamp", "desc"));
    onSnapshot(q, (snap) => {
        const full = snap.docs.map(d => ({id: d.id, ...d.data()}));
        const sliced = full.slice((adminPage-1)*PAGE_SIZE, adminPage*PAGE_SIZE);
        document.getElementById('adminBody').innerHTML = sliced.map(r => `
            <tr><td>${r.user}</td><td>₱${r.amount.toFixed(2)}</td><td>${r.method}: ${r.info}</td>
            <td>
                ${r.status === 'Pending' ? `
                    <button onclick="updateStat('${r.id}','Paid')" style="background:green;padding:5px;width:30px">✓</button>
                    <button onclick="updateStat('${r.id}','Denied')" style="background:red;padding:5px;width:30px">X</button>
                ` : r.status}
            </td></tr>
        `).join('');
    });
...
window.updateStat = async (id, stat) => { await updateDoc(doc(db, "withdrawals", id), { status: stat }); };

window.changePage = (type, dir) => {
    if(type === 'user') { userPage = Math.max(1, userPage + dir); document.getElementById('userPageNum').innerText = userPage; fetchHistory(); }
    else { adminPage = Math.max(1, adminPage + dir); document.getElementById('adminPageNum').innerText = adminPage; fetchAdmin(); }
};
