...
// Removed loading overlay related functions (showLoading, hideLoading, displayError with overlay)
    ...
    if (user) {
        ...
        try {
            ...
            if (doc.exists) {
                ...
                if (!telegramUserId) {
                    // This scenario should ideally not happen if Telegram Web App is mandatory
                    // but as a fallback, prevent creating an empty profile without basic TG ID.
...
function initAppFeatures() {
    ...
    // Ad CPM values are configured on the Monetag platform, not directly in this code.
    // To optimize CPM, ensure good ad placement, user engagement, and traffic quality.
...
async function runTask(id, zone, type = 'interstitial') { 
    ...
    if (typeof adFn === 'function') {
        try {
            ...
            // Ad displayed/completed. Now check if authenticated to enable claiming.
            if (firebaseUserId) {
                currentTaskId = id; 
